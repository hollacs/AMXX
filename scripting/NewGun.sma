...
#define WEAPON_CUSTOM_ZOOM // You need REMOVE_SECONDARY_ATTACK to make this work
#define WEAPON_ZOOM1 55 // AUG/SG552=55, Snipers=40
#define WEAPON_ZOOM2 90 // Snipers=15, AWP=10
#define WEAPON_ZOOM_DELAY 0.3
...
public plugin_init()
{
	register_plugin("New Gun for Sniper", VERSION, "Colgate");
...
public OnWeaponGetMaxSpeed(ent)
{
	...
	client_print(0, print_chat, "player = %d, fov = %d", player, pev(player, pev_fov));
...
public OnWeaponPrimaryAttack_Post(ent)
{
	...
	client_print(0, print_chat, "fov = %d", pev(player, pev_fov));
	...
	set_pev(player, pev_punchangle, punchAngle);
#elseif WEAPON_ID == CSW_M3 || WEAPON_ID == CSW_XM1014
	new Float:punchAngle[3];
	pev(player, pev_punchangle, punchAngle);
...
{	
	...
	if (getPlayerData(player, "m_rgAmmo", ammoType) <= 0 || clip == WEAPON_MAXCLIP)
		return HAM_SUPERCEDE;
	
	if (getWeaponDataF(ent, "m_flNextPrimaryAttack") > get_gametime())
		return HAM_SUPERCEDE;
	
	g_clip = clip;
	g_reload = getWeaponData(ent, "m_fInSpecialReload");
...
public OnWeaponReload_Post(ent)
{
	...
	if (g_reload == 0)
	{
		sendWeaponAnim(player, WEAPON_ANIM_START_RELOAD);
	}
	else if (g_reload == 1 && getWeaponData(ent, "m_fInSpecialReload") == 2)
	{
		sendWeaponAnim(player, WEAPON_ANIM_RELOAD);
		setWeaponDataF(ent, "m_flTimeWeaponIdle", get_gametime() + WEAPON_RELOAD_DELAY);
		setWeaponDataF(ent, "m_flNextReload", get_gametime() + WEAPON_RELOAD_DELAY);
	}
	else
	{
		g_clip++;
...
#if defined WEAPON_CUSTOM_ZOOM
	if ((pev(player, pev_button) & IN_ATTACK2) && getWeaponDataF(ent, "m_flNextSecondaryAttack") <= get_gametime())
	{
		new fov = pev(player, pev_fov);
		if (fov == DEFAULT_FOV)
			fov = WEAPON_ZOOM1;
		else if (fov == WEAPON_ZOOM1)
			fov = WEAPON_ZOOM2;
		else
			fov = DEFAULT_FOV;
		
		setPlayerData(player, "m_iFOV", fov);
		set_pev(player, pev_fov, fov);
		
		ExecuteHamB(Ham_CS_Player_ResetMaxSpeed, player);
		
		emit_sound(player, CHAN_ITEM, "weapons/zoom.wav", 0.20, 2.40, 0, 100);
		setWeaponDataF(ent, "m_flNextSecondaryAttack", get_gametime() + WEAPON_ZOOM_DELAY);
	}
#endif
...
public OnWeaponIdle(ent)
{
...
#if WEAPON_ID == CSW_M3 || WEAPON_ID == CSW_XM1014
	if (g_pumpTime >= 0.0 && g_pumpTime < get_gametime())
	{
		g_pumpTime = 0.0;
	}
	...
	if (getWeaponDataF(ent, "m_flTimeWeaponIdle") < get_gametime())
	{
		...
		if (clip == 0 && getWeaponData(ent, "m_fInSpecialReload") == 0 && ammoAmount)
		{
			ExecuteHamB(Ham_Weapon_Reload, ent);
		}
		else if (getWeaponData(ent, "m_fInSpecialReload") != 0)
		{
			if (clip != WEAPON_MAXCLIP && ammoAmount)
			{
				ExecuteHamB(Ham_Weapon_Reload, ent);
			}
			else
			{
				sendWeaponAnim(player, WEAPON_ANIM_PUMP);
				
				setWeaponData(ent, "m_fInSpecialReload", 0);
				setWeaponDataF(ent, "m_flTimeWeaponIdle", get_gametime() + 1.5);
			}
		}
		else
		{
...
#elseif WEAPON_ID == CSW_G3SG1 || WEAPON_ID == CSW_SG550 || WEAPON_ID == CSW_AWP || WEAPON_ID == CSW_SCOUT
	if (getWeaponDataF(ent, "m_flTimeWeaponIdle") <= get_gametime())
	{
		if (getWeaponData(ent, "m_iClip"))
		{
			setWeaponDataF(ent, "m_flTimeWeaponIdle", get_gametime() + 60.0);
			sendWeaponAnim(player, WEAPON_ANIM_IDLE);
		}
	}
#else
