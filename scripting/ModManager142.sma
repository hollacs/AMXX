...
public plugin_precache()
{
	...
	if (g_currentMod != -1)
	{
		// current map is not in current mod
		if (!isMapInMod(currentMap, g_currentMod))
		{
			new mod = findAvailableMod(currentMap);
			setNextMod(mod);
...
stock findAvailableMod(const currentMap[])
{
	// find a available mod
...
public plugin_init()
{
	...
	register_concmd("mm_votemap", "CmdVoteMap", ADMIN_MAP);
...
{
	getReadyToVote();
	client_print_color(0, print_team_default, "^4[HKGSE] ^1ADMIN 強制開始了投票.");
...
public ShowModVote()
{
	...
	for (new player = 1, i; player <= g_maxClients; player++)
	{
		...
		{
			len += formatex(menu[len], 511-len, "^n\y9. \w繼續 %a^n", ArrayGetStringHandle(g_modName, g_currentMod));
			keys |= MENU_KEY_9;
		}
...
public ShowModVoteResult()
{
	...
	for (new i = 0; i < g_numChoices; i++)
	{
		...
		len += formatex(menu[len], 511-len, "\y%d. \w%a", i+1, ArrayGetStringHandle(g_modName, mod));
	...
	if (g_menuChoices[ID_EXTEND])
	{
		len += formatex(menu[len], 511-len, "^n\y9. \w%a", ArrayGetStringHandle(g_modName, g_currentMod));
...
public ShowMapVote()
{
	...
	for (new player = 1, i; player <= g_maxClients; player++)
	{
		...
		if (g_menuChoices[ID_EXTEND])
		{
			...
			len += formatex(menu[len], 511-len, "^n\y9. \w繼續 %s^n", mapName);
			keys |= MENU_KEY_9;
		}
...
{
	if (key == 9 || g_voting != 2 || g_hasVoted[id])
		return;
...
public ShowMapVoteResult()
{
	...
	{
		get_mapname(mapName, charsmax(mapName));
		len += formatex(menu[len], 511-len, "^n\y9. \w%s", mapName);
